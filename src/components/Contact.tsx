...
export default function Contact() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTimeout(() => {
        setStatus("idle");
        setForm({ name:"", email:"", company:"", role:"retailer", message:"" });
      }, 6000);
    } catch {
  ...
  return (
    <section id="contact" style={{padding:"104px 0",background:"var(--bg)",position:"relative",overflow:"hidden"}}>
          ...
          <div style={{background:"rgba(139,92,246,.06)",borderRadius:18,border:"1px solid rgba(139,92,246,.18)",padding:36,backdropFilter:"blur(12px)"}}>
            ...
            {status === "success" && (
              <div style={{textAlign:"center",padding:"52px 0"}}>
                ...
                <p style={{color:"rgba(255,255,255,.45)",fontSize:15}}>
                  We&apos;ll reach out to you at <strong style={{color:"#a78bfa"}}>{form.email || "your email"}</strong> shortly.
                </p>
              </div>
            )}
            ...
            {status === "error" && (
              <div style={{textAlign:"center",padding:"52px 0"}}>
                <div style={{width:64,height:64,borderRadius:"50%",background:"rgba(248,113,113,.1)",border:"1px solid rgba(248,113,113,.2)",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 20px"}}>
                ...
                <h3 className="hf" style={{fontWeight:700,color:"#fff",fontSize:20,marginBottom:8}}>Something went wrong</h3>
                <p style={{color:"rgba(255,255,255,.45)",fontSize:14}}>
                  Please email us directly at <strong style={{color:"#a78bfa"}}>connect@namuste.com</strong>
                </p>
              </div>
            )}
                ...
                <form onSubmit={handleSubmit} style={{display:"flex",flexDirection:"column",gap:13}}>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
                    <div>
                      ...
                      <input required type="text" value={form.name}
                        ...
                        placeholder="Your name" style={inp} onFocus={fi} onBlur={fo}
                      />
                    </div>
                    <div>
                      ...
                      <input required type="email" value={form.email}
                        ...
                        placeholder="you@company.com" style={inp} onFocus={fi} onBlur={fo}
                      />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <input type="text" value={form.company}
                      ...
                      placeholder="Your Business Name" style={inp} onFocus={fi} onBlur={fo}
                    />
                  </div>
                  ...
                  >
                    {status === "loading" ? (
                      <>
                        <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2" strokeLinecap="round"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg>
                        Sending...
                      </>
                    ) : (
                      <>
      ...
      <style>{`
        @media(max-width:900px){.cg{grid-template-columns:1fr!important;gap:48px!important}}
